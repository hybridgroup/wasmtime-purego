...
type MemoryType struct {
	_ptr   uintptr // *C.wasm_memorytype_t
	_owner interface{}
}
...
func NewMemoryType(min uint32, has_max bool, max uint32, shared bool) *MemoryType {
	...
	ptr := wasmtime_memorytype_new(uint64(min), has_max, uint64(max), false, shared)
...
func NewMemoryType64(min uint64, has_max bool, max uint64, shared bool) *MemoryType {
	...
	ptr := wasmtime_memorytype_new(min, has_max, max, true, shared)
	return mkMemoryType(ptr, nil)
}

func mkMemoryType(ptr uintptr, owner interface{}) *MemoryType {
...
func (ty *MemoryType) ptr() uintptr {
	ret := ty._ptr
	if ret == uintptr(0) {
...
func (ty *MemoryType) Maximum() (bool, uint64) {
	size := uint64(0)
	present := wasmtime_memorytype_maximum(ty.ptr(), &size)
	runtime.KeepAlive(ty)
...
func (ty *MemoryType) Is64() bool {
	ok := wasmtime_memorytype_is64(ty.ptr())
	runtime.KeepAlive(ty)
	return bool(ok)
}
...
func (ty *MemoryType) IsShared() bool {
	ok := wasmtime_memorytype_isshared(ty.ptr())
	runtime.KeepAlive(ty)
	return bool(ok)
}
...
func (ty *MemoryType) AsExternType() *ExternType {
	ptr := wasm_memorytype_as_externtype_const(ty.ptr())
